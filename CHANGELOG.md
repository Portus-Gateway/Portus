...
- **AI gateway** (opt-in, `aiGateway.enabled: true`). Three CRDs turn a Gateway into a front for LLM providers while clients keep speaking the provider's native API: `AIProvider` (an upstream such as the Anthropic Messages or OpenAI chat API, its credential in a Secret), `AIRoute` (matches on request body fields such as `model` and `stream` as well as path and headers, rewrites `Host`, swaps the client's key for the provider's) and `AIUsagePolicy` (a token budget per key, tenant or route over an hourly, daily or monthly window). The data plane scans the JSON body with a streaming top-level scanner (8 MiB cap, 413 above it), meters input, output and cache tokens from JSON and SSE responses of both dialects, and enforces `requireApiKey` locally from a key snapshot. Refusals are shaped like the provider's own errors (401 `authentication_error`, 403 `permission_error`, 429 with `Retry-After` and `x-portus-tokens-remaining`). Needs the Rama network stack.
- **`portus-ledger`**, the AI gateway's companion service (image `ghcr.io/portus-gateway/ledger`, deployed by the chart when `aiGateway.enabled`). It issues and imports API keys (hashes only, streamed to the data planes as versioned snapshots), keeps budgets as a shared counter the data planes sync every second, and stores usage records in SQLite (`/export.jsonl`, `/v1/summary`, `/v1/keys` behind a chart-generated admin token, Prometheus `/metrics`). The data plane never calls it on the request path. Example manifests under `deploy/examples/ai-gateway/`.
- Ordinary policies (rate limits, timeouts, retries, CORS, IP allow lists, body size limits, …) accept `targetRef.kind: AIRoute`.

### Changed

- **Rama is the release network stack.** `dataplane.networkStack` defaults to `rama`; the release image carries both stacks and `networkStack: pingora` still selects Pingora at run time. Gateway API v1.6.2 conformance 130/130 on the release image with the stack unset. Against Pingora on the same core, machine and pods, Rama was 3–31 % ahead on every payload rung and used 2–2.6× less memory (single round, 2026-09-15); three-round medians follow in 0.2.5.
- **Rama pods drain on SIGTERM.** New connections stop being accepted, `/readyz` answers 503, idle keep-alive connections close, requests in flight finish, then the pod exits 0 (exit 1 after `PORTUS_DRAIN_SECONDS`, default 25). The provisioner sets `terminationGracePeriodSeconds: 30` (was 5).
- The Rama stack applies `RequestBodySizeLimitPolicy` to streaming request bodies too: a chunked body that crosses the limit answers 413 and closes the connection instead of being forwarded.
- Crate versions follow the chart (0.2.4).
...
- **Request decisions live in the core.** `portus-dataplane-core::plan::plan_request` makes every pre-backend decision (listener isolation, misdirected requests, redirects, CORS, IP filters, body limits, authentication, weighted backend choice, timeouts, rewrites, mirrors, rate limits, circuit breakers, connection limits) once per request from stack-neutral request facts; a network stack only extracts the facts, writes the reply or forwards as planned. Request matching no longer depends on the `http` crate's header and method types.
- **Rama stack** (`cargo build --features rama`, image `DATAPLANE_FEATURES=rama`, the default for both): the same core on [Rama](https://github.com/plabayo/rama) 0.4, selected at run time with `dataplane.networkStack=rama`. Passes Gateway API v1.6.2 conformance 130/130 (all five profiles) as of 2026-09-15; WebSocket upgrades are spliced, and requests that arrive over HTTP/2 forward their `:authority` as `Host`.
- **`dataplane.networkStack`** (`PORTUS_NETWORK_STACK`) names the stack the pods serve on; a value the image does not carry fails the pod at start with a log line naming it.
