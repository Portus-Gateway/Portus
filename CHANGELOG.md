...
- **Experimental Rama stack** (`cargo build --features rama`, image `DATAPLANE_FEATURES=rama`): the same core on [Rama](https://github.com/plabayo/rama) 0.4, selected at run time with `dataplane.networkStack=rama`. Not built into release images; it exists to be compared against Pingora on the same benchmarks and conformance suite. Passes Gateway API v1.6.2 conformance 130/130 (all five profiles) as of 2026-09-15; WebSocket upgrades are spliced, and requests that arrive over HTTP/2 forward their `:authority` as `Host`.
