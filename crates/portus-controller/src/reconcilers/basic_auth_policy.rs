//! BasicAuthPolicy reconciler.
//!
//! Watches BasicAuthPolicy resources, validates targetRef, validates Secret
...
use crate::policy_types::BasicAuthPolicy;
use crate::status;
use crate::store::{BasicAuthPolicyState, ConfigStore, NamespacedName, PolicyTargetKey};
...
pub fn reconcile_inner(
    ...
    let secret_name = &policy.spec.basic_auth.secret_ref.name;
        ...
        )
    {
        ...
        store.basic_auth_policies.insert(my_key, state);
    ...
    store.basic_auth_policies.insert(my_key.clone(), state);

    let mut conditions = Vec::new();

    if !secret_exists {
        // Secret not found -- reject the policy
        if let Some(mut entry) = store.basic_auth_policies.get_mut(&my_key) {
        ...
        resolve_conflicts(&my_key, &target, &store.basic_auth_policies);
        ...
        if accepted {
            ...
            let winner_msg = find_winner_key(&target, &my_key, &store.basic_auth_policies)
                .map(|k| format!("Older BasicAuthPolicy {} takes precedence", k))
...
/// Main reconcile function for BasicAuthPolicy resources.
pub async fn reconcile_basic_auth_policy(
    policy: Arc<BasicAuthPolicy>,
    ctx: Arc<ReconcileContext>,
) -> Result<Action, ReconcileError> {
    let desired_conditions = super::policy_common::reconcile_publishing(
        &ctx.store,
        &ctx.store.basic_auth_policies,
        "BasicAuthPolicy",
        &policy.metadata,
    ...
    let api: Api<BasicAuthPolicy> = Api::namespaced(ctx.client.clone(), namespace);
    ...
    {
        log::warn!(
            "failed to write BasicAuthPolicy status for {}/{}: {}; will retry on next reconcile",
            namespace,
            name,
            e
        );
...
mod tests {
    use super::*;
    use crate::policy_types::{BasicAuthPolicySpec, BasicAuthSpec, PolicyTargetRef, SecretRef};
    use crate::store::{ConfigStore, SecretState};
    use k8s_openapi::apimachinery::pkg::apis::meta::v1::ObjectMeta;
    use std::collections::HashMap;

    fn make_policy(name: &str, ns: &str, target_name: &str, secret_name: &str) -> BasicAuthPolicy {
        BasicAuthPolicy {
            metadata: ObjectMeta {
    ...
    fn test_accepted_when_secret_exists() {
        ...
        let policy = make_policy("ba1", "default", "my-route", "my-secret");
    ...
    fn test_rejected_when_secret_missing() {
        let store = ConfigStore::new();
        let policy = make_policy("ba1", "default", "my-route", "missing-secret");
        let conditions = reconcile_inner(&policy, &store).unwrap();
    ...
    fn test_conflict_older_wins() {
        let store = ConfigStore::new();
        // Need a secret for policies to be accepted
        store.secrets.insert(
            NamespacedName {
                namespace: "default".to_string(),
                name: "my-secret".to_string(),
        ...
        let mut older = make_policy("ba-older", "default", "my-route", "my-secret");
        ...
        let mut newer = make_policy("ba-newer", "default", "my-route", "my-secret");
        ...
        let older_key = NamespacedName {
            namespace: "default".to_string(),
            name: "ba-older".to_string(),
        };
        let newer_key = NamespacedName {
            namespace: "default".to_string(),
            name: "ba-newer".to_string(),
        };

        let older_state = store.basic_auth_policies.get(&older_key).unwrap();
        let newer_state = store.basic_auth_policies.get(&newer_key).unwrap();
    ...
    fn test_default_realm() {
        ...
        let stored = store.basic_auth_policies.get(&key).unwrap();
        assert_eq!(stored.realm, "Restricted");
    }
}
