//! CORSPolicy reconciler.

use super::policy_common::{find_winner_key, resolve_conflicts};
use super::{ReconcileContext, ReconcileError};
use crate::policy_types::CORSPolicy;
use crate::status;
use crate::store::{CORSPolicyState, ConfigStore, NamespacedName, PolicyTargetKey};
...
pub fn reconcile_inner(
    ...
    let cors = &policy.spec.cors;
    let state = CORSPolicyState {
        target: target.clone(),
    ...
    store.cors_policies.insert(my_key.clone(), state);
    resolve_conflicts(&my_key, &target, &store.cors_policies);
    ...
    if accepted {
        ...
        let winner_msg = find_winner_key(&target, &my_key, &store.cors_policies)
            .map(|k| format!("Older CORSPolicy {} takes precedence", k))
...
pub async fn reconcile_cors_policy(
    ...
    let api: Api<CORSPolicy> = Api::namespaced(ctx.client.clone(), namespace);
    ...
    {
        log::warn!(
            "failed to write CORSPolicy status for {}/{}: {}; will retry on next reconcile",
            namespace,
            name,
            e
        );
...
mod tests {
    use super::*;
    use crate::policy_types::{PolicyTargetRef, CORSPolicySpec, CORSSpec};
    use crate::store::ConfigStore;
    use k8s_openapi::apimachinery::pkg::apis::meta::v1::{ObjectMeta, Time};

    fn make_policy(name: &str, ns: &str, target_name: &str) -> CORSPolicy {
        CORSPolicy {
            metadata: ObjectMeta {
    ...
    fn test_single_policy_accepted() {
        ...
        let policy = make_policy("cors1", "default", "my-route");
    ...
    fn test_different_targets_no_conflict() {
        let store = ConfigStore::new();
        let p1 = make_policy("cors1", "default", "route-a");
        let p2 = make_policy("cors2", "default", "route-b");
    ...
    fn test_conflict_older_wins() {
        ...
        let mut older = make_policy("cors-old", "default", "my-route");
        older.metadata.creation_timestamp = Some(older_ts);
        let mut newer = make_policy("cors-new", "default", "my-route");
        ...
        let old_key = NamespacedName {
            ...
            name: "cors-old".to_string(),
        };
        let new_key = NamespacedName {
            namespace: "default".to_string(),
            name: "cors-new".to_string(),
        };
        assert!(store.cors_policies.get(&old_key).unwrap().accepted);
        assert!(!store.cors_policies.get(&new_key).unwrap().accepted);
    }
}
