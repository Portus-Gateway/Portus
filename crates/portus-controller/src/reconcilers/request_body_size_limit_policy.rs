//! RequestBodySizeLimitPolicy reconciler.

use super::policy_common::{find_winner_key, resolve_conflicts};
use super::{ReconcileContext, ReconcileError};
use crate::policy_types::RequestBodySizeLimitPolicy;
use crate::status;
use crate::store::{ConfigStore, NamespacedName, PolicyTargetKey, RequestBodySizeLimitPolicyState};
...
pub fn reconcile_inner(
    policy: &RequestBodySizeLimitPolicy,
    store: &ConfigStore,
    ...
    let state = RequestBodySizeLimitPolicyState {
        target: target.clone(),
        max_bytes: policy.spec.max_bytes,
        generation,
        creation_timestamp: creation_ts.clone(),
        accepted: true,
    };

    store.request_body_size_limit_policies.insert(my_key.clone(), state);
    resolve_conflicts(&my_key, &target, &store.request_body_size_limit_policies);

    let accepted = store
        .request_body_size_limit_policies
        .get(&my_key)
        .map(|s| s.accepted)
        .unwrap_or(false);
    ...
    if accepted {
        ...
        let winner_msg = find_winner_key(&target, &my_key, &store.request_body_size_limit_policies)
            .map(|k| format!("Older RequestBodySizeLimitPolicy {} takes precedence", k))
...
pub async fn reconcile_request_body_size_limit_policy(
    policy: Arc<RequestBodySizeLimitPolicy>,
    ctx: Arc<ReconcileContext>,
) -> Result<Action, ReconcileError> {
    let desired_conditions = super::policy_common::reconcile_publishing(
        &ctx.store,
        &ctx.store.request_body_size_limit_policies,
    ...
    let api: Api<RequestBodySizeLimitPolicy> = Api::namespaced(ctx.client.clone(), namespace);
...
mod tests {
    use super::*;
    use crate::policy_types::{PolicyTargetRef, RequestBodySizeLimitPolicySpec};
    use crate::store::ConfigStore;
    use k8s_openapi::apimachinery::pkg::apis::meta::v1::{ObjectMeta, Time};

    fn make_policy(name: &str, ns: &str, target_name: &str, max_bytes: u64) -> RequestBodySizeLimitPolicy {
        RequestBodySizeLimitPolicy {
            metadata: ObjectMeta {
            ...
            spec: RequestBodySizeLimitPolicySpec {
    ...
    fn test_single_policy_accepted() {
        ...
        let policy = make_policy("bsl1", "default", "my-route", 1_048_576);
    ...
    fn test_different_targets_no_conflict() {
        let store = ConfigStore::new();
        let p1 = make_policy("bsl1", "default", "route-a", 1_048_576);
        let p2 = make_policy("bsl2", "default", "route-b", 2_097_152);
    ...
    fn test_conflict_older_wins() {
        ...
        let mut older = make_policy("bsl-old", "default", "my-route", 1_048_576);
        older.metadata.creation_timestamp = Some(older_ts);
        let mut newer = make_policy("bsl-new", "default", "my-route", 2_097_152);
        ...
        let old_key = NamespacedName {
            ...
            name: "bsl-old".to_string(),
        };
        let new_key = NamespacedName {
            namespace: "default".to_string(),
            name: "bsl-new".to_string(),
        };
        assert!(store.request_body_size_limit_policies.get(&old_key).unwrap().accepted);
        assert!(!store.request_body_size_limit_policies.get(&new_key).unwrap().accepted);
    }
}
