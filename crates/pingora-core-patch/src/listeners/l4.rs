...
pub use crate::protocols::l4::listener::HandoffSource;
...
pub enum ServerAddress {
    ...
    /// Not a socket: connections are accepted by another loop in this process
    /// and handed over through the [`HandoffSource`]. The string is a label for
    /// logs and metrics (e.g. `handoff:https`). Never participates in the
    /// zero-downtime-upgrade fd table.
    Handoff(String, HandoffSource),
}
...
fn from_raw_fd(address: &ServerAddress, fd: i32) -> Result<Listener> {
    match address {
        ServerAddress::Handoff(label, _) => Error::e_explain(
            BindError,
            format!("hand-off endpoint {label} cannot be restored from an inherited fd"),
        ),
        #[cfg(unix)]
...
async fn bind(addr: &ServerAddress) -> Result<Listener> {
    match addr {
        ...
        ServerAddress::Handoff(_, source) => Ok(Listener::Handoff(source.clone())),
    }
}
...
impl ListenerEndpointBuilder {
    ...
    pub async fn listen(self, fds: Option<ListenFds>) -> Result<ListenerEndpoint> {
        ...
        let listener = if matches!(listen_addr, ServerAddress::Handoff(..)) {
            // Nothing to inherit or share: the channel lives in this process.
            bind(&listen_addr).await?
        } else if let Some(fds_table) = fds {
