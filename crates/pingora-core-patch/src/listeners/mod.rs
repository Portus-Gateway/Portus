...
pub use l4::{HandoffSource, ServerAddress, TcpSocketOptions};
...
impl ListenerConfig {
    ...
    pub fn tcp_socket_options(mut self, options: TcpSocketOptions) -> Self {
        match &mut self.l4 {
            ...
            #[cfg(unix)]
            ServerAddress::Uds(_, _) | ServerAddress::Handoff(_, _) => {
                panic!("TCP socket options can only be set on TCP endpoints")
            }
            #[cfg(not(unix))]
            ServerAddress::Handoff(_, _) => {
                panic!("TCP socket options can only be set on TCP endpoints")
            }
        }
        self
    ...
    pub fn permissions(mut self, permissions: Permissions) -> Self {
        match &mut self.l4 {
            ...
            ServerAddress::Tcp(_, _) | ServerAddress::Handoff(_, _) => {
...
impl Listeners {
    ...
    /// Add a TLS endpoint whose connections are handed over from another accept
    /// loop in this process (see [`l4::HandoffSource`]) instead of being bound to
    /// a socket. `label` names the endpoint in logs; it must be unique among the
    /// endpoints of this listener collection.
    pub fn add_tls_handoff(&mut self, label: &str, source: l4::HandoffSource, settings: TlsSettings) {
        self.add_endpoint(ServerAddress::Handoff(label.into(), source), Some(settings));
    }

    /// Add a plaintext endpoint fed by a [`l4::HandoffSource`].
    pub fn add_handoff(&mut self, label: &str, source: l4::HandoffSource) {
        self.add_address(ServerAddress::Handoff(label.into(), source));
    }
...
mod test {
    ...
    #[tokio::test]
    async fn test_listen_handoff_keeps_peer_addr_and_bytes() {
        use tokio::io::{AsyncReadExt, AsyncWriteExt};

        // A hand-off endpoint accepts sockets delivered on a channel instead of
        // binding a port. The socket digest must describe the real client and
        // any bytes already in the kernel buffer must still be readable.
        let (tx, rx) = tokio::sync::mpsc::channel(4);
        let mut listeners = Listeners::new();
        listeners.add_handoff("handoff:test", l4::HandoffSource::new(rx));
        let stacks = listeners
            .build(
                #[cfg(unix)]
                None,
            )
        ...
        assert_eq!(stacks[0].as_str(), "handoff:test");
        let stack = stacks.into_iter().next().unwrap();

        // Real socket pair via an ephemeral listener, as the SNI mux would do.
        ...
        tx.send(accepted.into_std().unwrap()).await.unwrap();

        let uninit = stack.accept().await.unwrap();
        assert_eq!(
            uninit.peer_addr().and_then(|a| a.as_inet().copied()),
            Some(client_addr),
            "digest must carry the real client"
        );
        let mut io = uninit.handshake().await.unwrap();
        let mut buf = [0u8; 5];
        io.read_exact(&mut buf).await.unwrap();
        assert_eq!(&buf, b"hello");
        io.write_all(b"world").await.unwrap();
        io.flush().await.unwrap(); // Pingora streams buffer writes
        let mut back = [0u8; 5];
    ...
    #[tokio::test]
    async fn test_listen_handoff_parks_when_senders_are_gone() {
        // With every sender dropped, accept() must not spin with errors; it
        // simply never resolves (the service's shutdown signal ends the loop).
        let (tx, rx) = tokio::sync::mpsc::channel::<std::net::TcpStream>(1);
        drop(tx);
        let mut listeners = Listeners::new();
        listeners.add_handoff("handoff:closed", l4::HandoffSource::new(rx));
        let stack = listeners
            .build(
                #[cfg(unix)]
                None,
            ...
            .next()
            .unwrap();
        let parked = tokio::time::timeout(Duration::from_millis(50), stack.accept()).await;
        assert!(parked.is_err(), "accept on a closed hand-off channel must park, not error");
    }
}
