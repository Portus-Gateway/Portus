...
/// Create a `WrappedX509` from raw DER bytes.
/// Used by BackendTLSPolicy to construct CA certificates for per-peer verification.
pub fn wrapped_x509_from_der(der: Vec<u8>) -> WrappedX509 {
    WrappedX509::new(der, parse_x509)
}

impl WrappedX509 {
    /// Get the raw DER-encoded certificate bytes.
    pub fn raw_der(&self) -> &[u8] {
        self.borrow_raw_cert()
    }
}
