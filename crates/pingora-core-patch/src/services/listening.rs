...
impl<A> Service<A> {
    ...
    /// Add a TLS endpoint fed by connections handed over from another accept
    /// loop in this process. See [`Listeners::add_tls_handoff`].
    pub fn add_tls_handoff(
        &mut self,
        label: &str,
        source: crate::listeners::HandoffSource,
        settings: TlsSettings,
    ) {
        self.listeners.add_tls_handoff(label, source, settings)
    }

    /// Add a plaintext endpoint fed by handed-over connections. See
    /// [`Listeners::add_handoff`].
    pub fn add_handoff(&mut self, label: &str, source: crate::listeners::HandoffSource) {
        self.listeners.add_handoff(label, source)
    }
