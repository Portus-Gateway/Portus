    ...
    P: Peer + Send + Sync,
{
    ...
    // BackendTLSPolicy SAN validation: if required_sans is set, install a custom
    // verifier that checks the peer cert's SANs after standard WebPKI validation.
    if let Some(ref required_sans) = peer.get_peer_options().and_then(|o| o.required_sans.as_ref()) {
        if !required_sans.is_empty() {
            let config = updated_config_opt.get_or_insert_with(|| {
            ...
            let san_verifier = Arc::new(SanCheckingVerifier {
                delegate,
                required_sans: Arc::clone(required_sans),
            });
            config.dangerous().set_certificate_verifier(san_verifier);
        }
    }
...
/// BackendTLSPolicy SAN verifier: delegates to WebPkiServerVerifier for standard
/// certificate chain + hostname validation, then additionally checks that the peer
/// certificate contains at least one SubjectAltName matching the required list.
#[derive(Debug)]
...
impl RusTlsServerCertVerifier for SanCheckingVerifier {
    fn verify_server_cert(
        &self,
        ...
        // First: standard WebPKI validation (CA chain + hostname)
        self.delegate.verify_server_cert(end_entity, intermediates, server_name, ocsp, now)?;

        // Then: check required SubjectAltNames
        let cert_der = end_entity.as_ref();
        let san_ok = match x509_parser::parse_x509_certificate(cert_der) {
            Ok((_, parsed)) => {
                let cert_sans: Vec<(String, String)> = parsed
                        ...
                        ext.value
                            ...
                            .filter_map(|gn| match gn {
                                x509_parser::prelude::GeneralName::DNSName(dns) => {
                                    Some(("Hostname".to_string(), dns.to_string()))
                                }
                                x509_parser::prelude::GeneralName::URI(uri) => {
                                    Some(("URI".to_string(), uri.to_string()))
                                }
                                _ => None,
                            })
                            .collect()
                    ...
                    .unwrap_or_default();
                // At least one required SAN must match a cert SAN
                self.required_sans.iter().any(|(req_type, req_value)| {
                    cert_sans.iter().any(|(cert_type, cert_value)| {
                        cert_type == req_type && cert_value == req_value
                    })
                })
            }
            Err(_) => false,
