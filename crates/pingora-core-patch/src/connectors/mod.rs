...
mod tests {
    ...
    // macOS lets a socket bound to 127.0.0.1:0 connect to the unroutable test peer and

    // hang instead of failing fast; upstream notes the same ("some systems will allow...").

    #[cfg_attr(target_os = "macos", ignore = "environment-dependent: macOS does not fail the bind-to connect fast")]
