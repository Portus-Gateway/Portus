...
#![warn(clippy::all)]
// Upstream code trips these newer lints; allowed here (in-source, because the
// `warn(clippy::all)` above outranks Cargo `[lints]`) so the workspace stays
// clippy-clean without widening the patch diff.
