...
pub struct SslDigest {
    ...
    /// The SNI (Server Name Indication) provided by the client in the TLS
    /// ClientHello. Populated on accepted (server) rustls connections; `None`
    /// on client connections or when the client did not send SNI.
