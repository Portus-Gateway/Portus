...
/// Perform the TLS handshake letting `chooser` pick the `ServerConfig` once the
/// ClientHello is known (see [`crate::listeners::tls::ServerConfigChooser`]).
pub async fn handshake_with_chooser<S: IO>(
    acceptor: &Acceptor,
    io: S,
    chooser: &dyn crate::listeners::tls::ServerConfigChooser,
    local_addr: Option<&crate::protocols::l4::socket::SocketAddr>,
) -> Result<TlsStream<S>> {
    let mut stream = TlsStream::from_lazy_acceptor(acceptor, io, chooser, local_addr)
        .await
        .explain_err(TLSHandshakeFailure, |e| format!("tls stream error: {e}"))?;
    stream
        .accept()
