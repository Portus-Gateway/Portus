    ...
    T: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    /// Like [`Self::from_acceptor`] but reads the ClientHello first and lets
    /// `chooser` select the `ServerConfig` for this connection.
    pub(crate) async fn from_lazy_acceptor(
        acceptor: &Acceptor,
        stream: T,
        chooser: &dyn crate::listeners::tls::ServerConfigChooser,
        local_addr: Option<&crate::protocols::l4::socket::SocketAddr>,
    ) -> Result<Self> {
        let tls = InnerStream::from_lazy_acceptor(acceptor, stream, chooser, local_addr)
            .await
            .explain_err(TLSHandshakeFailure, |e| format!("tls stream error: {e}"))?;
...
impl<T: AsyncRead + AsyncWrite + Unpin> InnerStream<T> {
    ...
    /// Read the ClientHello, ask `chooser` for the config, then continue the
    /// handshake exactly as [`Self::from_acceptor`] would.
    pub(crate) async fn from_lazy_acceptor(
        acceptor: &Acceptor,
        stream: T,
        chooser: &dyn crate::listeners::tls::ServerConfigChooser,
        ...
        .await
        .explain_err(TLSHandshakeFailure, |e| format!("tls client hello error: {e}"))?;
        let config = chooser
            .choose(local_addr, &start.client_hello())
            .unwrap_or_else(|| Arc::clone(acceptor.default_config()));
        let accept = start.into_stream(config);
...
impl SslDigest {
    fn from_stream<T>(stream: &Option<RusTlsStream<T>>) -> Self {
        ...
        let mut digest = SslDigest::new(cipher, version, organization, serial_number, cert_digest);
        digest.sni = sni;
        digest
    }
}
