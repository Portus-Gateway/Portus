...
/// Returns true if a header name (lowercase) is sensitive and should be redacted in logs.
fn is_sensitive_header(name: &str) -> bool {
    SENSITIVE_HEADERS.contains(&name)
}

/// Format an `http::HeaderMap` for logging, replacing sensitive header values with `[REDACTED]`.
pub fn redact_headers(headers: &http::HeaderMap) -> String {
...
/// Redact sensitive header values from raw HTTP wire bytes (UTF-8 text).
///
/// Scans line-by-line for header lines matching sensitive names and replaces the value portion.
pub fn redact_wire_headers(raw: &str) -> String {
...
mod test {
    ...
    #[test]
    fn test_redact_headers_proxy_auth_and_set_cookie() {
        let mut headers = http::HeaderMap::new();
        headers.insert(
            "proxy-authorization",
            "Basic dXNlcjpwYXNz".parse().unwrap(),
        );
        headers.insert("set-cookie", "id=abc; Path=/".parse().unwrap());
        headers.insert("x-custom", "visible".parse().unwrap());

        let result = redact_headers(&headers);
        assert!(result.contains("proxy-authorization: [REDACTED]"));
        assert!(result.contains("set-cookie: [REDACTED]"));
        assert!(result.contains("x-custom: visible"));
        assert!(!result.contains("dXNlcjpwYXNz"));
        assert!(!result.contains("id=abc"));
    }

    #[test]
    fn test_redact_wire_headers() {
        let raw = "GET / HTTP/1.1\r\nHost: example.com\r\nAuthorization: Bearer token\r\nCookie: session=xyz\r\n\r\n";
        let result = redact_wire_headers(raw);
        assert!(result.contains("Host: example.com"));
        assert!(result.contains("Authorization: [REDACTED]"));
        assert!(result.contains("Cookie: [REDACTED]"));
    ...
    #[test]
    fn test_redact_wire_headers_response() {
