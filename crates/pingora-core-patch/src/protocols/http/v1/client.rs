...
impl HttpSession {
    ...
    pub async fn write_request_header(&mut self, req: Box<RequestHeader>) -> Result<usize> {
        ...
        trace!(
            ...
            redact_wire_headers(str::from_utf8(to_wire.as_ref()).unwrap_or("[non-utf8]"))
        );
    ...
    pub async fn read_response_task(&mut self) -> Result<HttpTask> {
        if self.should_read_resp_header() {
            ...
            debug!(
                "Response header: status={}, headers=[{}]",
                resp_header.status,
                redact_headers(&resp_header.headers)
            );
            trace!(
                "Raw Response header: {}",
                redact_wire_headers(str::from_utf8(self.get_headers_raw()).unwrap_or("[non-utf8]"))
            );
