...
use crate::protocols::http::v1::common::redact_headers;
    ...
    SV: ServeHttp + Send + Sync,
{
    async fn process_new_http(
        ...
        trace!(
            "Request: {} {} headers=[{}]",
            http.req_header().method,
            http.req_header().uri,
            redact_headers(&http.req_header().headers)
        );
        if *shutdown.borrow() {
    ...
    SV: ServeHttp + Send + Sync,
{
    async fn process_new_http(
        ...
        if let HttpTask::Header(ref h, end) = task {
            trace!(
                "HttpTask::Header(status={}, headers=[{}], end={end})",
                h.status,
                redact_headers(&h.headers),
            );
        }
